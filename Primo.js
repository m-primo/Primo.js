...
class Document {
    ...
    toggle() {
        for(var i = 0; i < this.elm.length; i++) {
            this.elm[i].style.display == 'none' ? this.elm[i].style.display = 'block' : this.elm[i].style.display = 'none';
        }
    }

    process(selfStyle = false) {
        for(var i = 0; i < this.elm.length; i++) {
            this.elm[i].innerHTML = 'Please Wait...';
            if(selfStyle) {
